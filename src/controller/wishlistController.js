...
export const addWishlist = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      const addWishlist = await wishlistModel.create({
        userId,
        productId,
        isCart: !!checkCart,
      });
...
export const removeFromWishlist = async (req, res) => {
  ...
  try {
    // Find the wishlist item directly
    const item = await wishlistModel.findOne({
      userId,
      productId,
      isDelete: false, // Only consider non-deleted ones
    });
    ...
    if (!item) {
      return response.error(
        ...
        req.languageCode,
        resStatusCode.NOT_FOUND,
        resMessage.PRODUCT_NOT_IN_WISHLIST
      );
    }
    ...
    return response.success(
      ...
      req.languageCode,
      resStatusCode.ACTION_COMPLETE,
      resMessage.WISHLIST_ITEM_REMOVED,
      item
    );
  } catch (err) {
