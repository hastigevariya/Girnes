...
export const adminGetAllMedia = async (req, res) => {
  ...
  try {
    ...
    return response.success(
      ...
      req.languageCode,
      resStatusCode.ACTION_COMPLETE,
      media.length ? resMessage.MEDIA_RETRIEVED : resMessage.NO_MEDIA_FOUND,
      media
    );
  } catch (err) {
    console.error(err);
...
export const getAllMedia = async (req, res) => {
  ...
  try {
    const media = await mediaModel
      .find({ type, isActive: true, isDelete: false })
      .sort({ createdAt: -1 });
...
export const addVideoUrl = async (req, res) => {
  ...
  try {
    const video = await mediaModel.create({
      file: vdoUrl,
      type: "vdo",
    });
  ...
  try {
    ...
    if (!existing) {
      ...
      return response.success(
        ...
        req.languageCode,
        resStatusCode.ACTION_COMPLETE,
        resMessage.SOCIAL_LINKS_CREATED,
        result
      );
    } else {
      ...
      return response.success(
        ...
        req.languageCode,
        resStatusCode.ACTION_COMPLETE,
        resMessage.SOCIAL_LINKS_UPDATED,
        result
      );
    }
  } catch (err) {
...
export const getSocialAccountURL = async (req, res) => {
  try {
    ...
    return response.success(
      ...
      resStatusCode.ACTION_COMPLETE,
      link
        ? resMessage.SOCIAL_LINKS_RETRIEVED
        : resMessage.NO_SOCIAL_LINKS_FOUND,
      link || {}
    );
  } catch (err) {
